...
const Sidebar = ({sidebar, setSidebar}) => {
  ...
  return (
    <div className={`w-72 bg-white border-r border-gray-200 flex flex-col justify-between
    items-center max-sm:absolute top-14 bottom-0 ${sidebar ? 'translate-x-0' : 'max-sm:-translate-x-full'} transition-all duration-300 ease-in-out`}>
        <div className='my-7 w-full'>
            ...
            <div className='px-7 mt-5 text-sm text-gray-600 font-medium'>
                {navItems.map(({to, label, Icon})=> (
                    ...
                    ${isActive ? 'bg-gradient-to-r from-[#022366] to-[#3C81F6] text-white' : '' }`}>
                        {({ isActive })=> (
                            <>
